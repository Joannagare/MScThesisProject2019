...
for t in grid:
        allcatgrids.append(listToStringWithoutBrackets(gridpath)+t)

with open('newin00.list', 'a') as output:
        for q in range(len(finallist)):
                for i in range(len(chromosomes)):
                        output.write(str(chromosomes[i])+"\t"+str(positions[i])+"\t"+finallist[q]+"\t"+allcatgrids[q]+"\t"+listToStringWithoutBrackets(samplelistdir00)+NameListToExportNames00[i]+"\n")
#.join(str(j).join("\t".join.vcf.join("\t",grid,"\t",sample,"\n"    \n")     

with open('newin11.list', 'a') as output:
        for a in range(len(finallist)):   
                for u in range(len(chromosomes)):
                        output.write(str(chromosomes[u])+"\t"+str(positions[u])+"\t"+finallist[a]+"\t"+allcatgrids[a]+"\t"+listToStringWithoutBrackets(samplelistdir11)+NameListToExportNames11[u]+"\n")
